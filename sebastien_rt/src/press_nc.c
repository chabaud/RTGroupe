...
static void	press_l(t_winfo *w, int key)
{
	...
	if (key == 13)
		w->light[(int)w->opt.lselect].pos = add_v(w->light[(int)w->opt.lselect].pos, mult_v(vrep[2], w->opt.speed));
	else if (key == 1)
		w->light[w->opt.lselect].pos = add_v(w->light[w->opt.lselect].pos, mult_v(vrep[2], -w->opt.speed));
	else if (key == 0)
		w->light[w->opt.lselect].pos = add_v(w->light[w->opt.lselect].pos, mult_v(vrep[0], -w->opt.speed));
	else if (key == 2)
		w->light[w->opt.lselect].pos = add_v(w->light[w->opt.lselect].pos, mult_v(vrep[0], w->opt.speed));
	else if (key == 15)
		w->light[w->opt.lselect].pos = add_v(w->light[w->opt.lselect].pos, mult_v(vrep[1], w->opt.speed));
	else if (key == 3)
		w->light[w->opt.lselect].pos = add_v(w->light[w->opt.lselect].pos, mult_v(vrep[1], -w->opt.speed));
}
...
void		press_noconsole(t_winfo *w, int key)
{
	...
	if (w->opt.oselect == -1)
		...
		if (!w->opt.trt)
			w->obj[w->opt.oselect].a = trans_p(w, key, w->obj[w->opt.oselect].a);
		else
